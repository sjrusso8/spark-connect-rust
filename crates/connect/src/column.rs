...
impl Column {
    ...
    pub fn astype(self, to_type: impl Into<CastToType>) -> Column {
        self.cast(to_type)
    }
    ...
    /// Equality comparsion. Cannot overload the '==' and return something other
    /// than a bool
    ...
    pub fn eq_null_safe(self, other: impl Into<Column>) -> Column {
        invoke_func("<=>", vec![self, other.into()])
    }
    ...
    pub fn lt_eq(self, other: impl Into<Column>) -> Column {
        invoke_func("<=", vec![self, other.into()])
    }
    ...
    /// Greater than or equal comparsion.
    pub fn gt_eq(self, other: impl Into<Column>) -> Column {
        invoke_func(">=", vec![self, other.into()])
    }

    /// Check if the current column’s values are between the specified lower and upper bounds, inclusive.
    pub fn between(self, lower_bound: impl Into<Column>, upper_bound: impl Into<Column>) -> Column {
        let upper = self.clone().lt_eq(upper_bound.into());
        self.gt_eq(lower_bound.into()).and(upper)
    }

    /// Logical AND comparsion. Cannot overload the '&&' and return something other
    /// than a bool
    pub fn and(self, other: impl Into<Column>) -> Column {
        invoke_func("and", vec![self, other.into()])
    }

    /// Logical OR comparsion.
    ...
    pub fn when(self, condition: impl Into<Column>, value: impl Into<Column>) -> Column {
        let Some(spark::expression::ExprType::UnresolvedFunction(func)) = self.expression.expr_type
        else {
            panic!("when() can only be applied on a Column previously generated by when() function, and cannot be applied once otherwise() is applied")
        };

        assert_eq!(func.function_name, "when", "when() can only be applied on a Column previously generated by when() function, and cannot be applied once otherwise() is applied");
        assert!(
            !self.else_value,
            "otherwise() has already been applied and when() not be applied again to the same Column"
        );

        let mut args = func.arguments;
        let mut expr_vec = VecExpression::from_iter(vec![condition.into(), value.into()]);

        args.append(&mut expr_vec.expr);

        invoke_func("when", args)
    }

    pub fn otherwise(self, value: impl Into<Column>) -> Column {
        let Some(spark::expression::ExprType::UnresolvedFunction(func)) = self.expression.expr_type
        else {
            panic!("otherwise() can only be applied on a Column previously generated by when() function, and cannot be applied once otherwise() is applied")
        };

        assert_eq!(func.function_name, "when", "otherwise() can only be applied on a Column previously generated by when() function, and cannot be applied once otherwise() is applied");
        assert!(
            !self.else_value,
            "otherwise() can only be applied once to a Column"
        );

        let mut args = func.arguments;
        args.push(value.into().expression);

        let mut col = invoke_func("when", args);
        col.else_value = true;
